...
The README is the first surface contributors see. It should identify the current app clearly near the top, explain how vendored ASDLC references relate to repo-specific docs, point contributors at the current runtime and verification commands, and keep screenshot refresh manual.
...
- **Update model:** manual refresh when the app UI changes materially
...
- Do not let the committed screenshot drift far from the current app UI.
...
- [ ] The README identifies the app as a Cloudflare Worker served with Wrangler near the top.
...
- The screenshot should continue to represent the current app surface closely enough to be useful.
- Screenshot support must remain manual unless a later ADR changes that rule.

### Verification

- **Screenshot refresh:** manual developer update when the app UI changes materially
...
- Then: they can tell quickly that the app is a Cloudflare Worker served with Wrangler and centered on server-rendered HTML
...
**Scenario: App UI changes materially**
