...
- a Durable Object server-sent event stream at `/events` for room state-change notifications, without interval polling fallback
...
- Browsers without event stream support do not receive passive live updates after the initial fragment refresh, but normal server-rendered pages and POST fallbacks still work.
