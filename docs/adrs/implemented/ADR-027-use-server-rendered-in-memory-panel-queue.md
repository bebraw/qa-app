...
- a Durable Object server-sent event stream at `/events` for room state-change notifications, with slow interval polling only when the browser does not support event streams
...
- If the browser does not support event streams, updates fall back to a slow polling interval rather than being purely push-based.
